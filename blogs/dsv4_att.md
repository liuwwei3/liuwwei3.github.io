...
**关键差异**：标准 MHA 中 $d_h = d / n_h$ 是硬约束（维度匹配要求）。MLA 中 K/V 从 $\mathbf{c}^{KV}$ 通过 $\mathbf{W}^{UK}$ 上投影解压得到，$d_h$ 是独立可选的超参数，不再受 $d/n_h$ 制约。DeepSeek 选择 $d_h=128$，此时 $n_h d_h = 128 \times 128 = 16384 \gg d = 7168$。输出投影 $\mathbf{W}^O$ 将 $n_h(d_h + d_h^R) = 128 \times 192 = 24576$ 维映射回 $d=7168$ 维。
...
MLA 的核心做法可以用 JPEG 压缩来类比：**训练时学习压缩-解压的参数，推理时只缓存压缩后的紧凑表示，读取时解压还原**。下面从 KV 联合压缩开始，逐步展开完整的 MLA 公式。
...
| **MQA 模式** | 自回归解码 | 所有头共享 `c^{KV}` | 多查询注意力 |

**转换方式**：MQA 模式下，$\mathbf{c}^{KV}$ 直接作为 key-value entry 而不按头解压。由于 $\mathbf{c}^{KV}$ 的维度 $d_c$ 通常等于 $d_h + d_h^R$（或相近），它与解耦位置 key $\mathbf{k}^R$ 拼接后可直接参与注意力——这等价于所有 Query 头共享同一组 KV，即 MQA。

### 4.8 训练与推理的完整流水线

前面分别介绍了 KV 压缩、解耦 RoPE、Query 压缩和 MHA/MQA 双模式，现在把它们串起来——看训练和推理时完整的数据流差异。

**压缩参数是怎么来的？** MLA 的下投影矩阵 $\mathbf{W}^{DKV}$ 和上投影矩阵 $\mathbf{W}^{UK}, \mathbf{W}^{UV}$ **都是在训练中端到端学习得到的**，不是推理时才"临时压缩"。训练和推理使用完全相同的参数，但走不同的数据路径：

|  | 训练时（MHA 模式） | 推理时（MQA 模式） |
|---|---|---|
...
| **Key/Value 粒度** | $\mathbf{k}_t^C$ 展开为每头 $\mathbf{k}_{t,i}^C$ | $\mathbf{c}_t^{KV}$ 直接作为共享 KV entry |
| | $\mathbf{v}_t^C$ 展开为每头 $\mathbf{v}_{t,i}^C$ | （MQA 模式，无需按头展开） |
| **注意力计算** | 每头独立计算，$n_h$ 组独立的 K,V | 所有头共享一组 KV，一次计算 |
| **梯度** | 反向传播更新 $\mathbf{W}^{DKV}, \mathbf{W}^{UK}, \mathbf{W}^{UV}$ | 参数已冻结，仅前向推理 |

**训练时**：走完整的上投影路径，每头得到独立的 $\mathbf{k}_{t,i}^C$ 和 $\mathbf{v}_{t,i}^C$，梯度通过上投影矩阵 $\mathbf{W}^{UK}, \mathbf{W}^{UV}$ 回传到压缩向量 $\mathbf{c}^{KV}$，再回传到下投影矩阵 $\mathbf{W}^{DKV}$。这样 $\mathbf{W}^{DKV}$ 学会了"如何将 $d$ 维压缩到 $d_c$ 维且保留最多注意力相关信息"。

**推理时**：不再需要展开（MQA 模式下 $\mathbf{c}^{KV}$ 直接参与注意力），也不需存储上投影的中间结果，仅缓存 $\mathbf{c}^{KV}$ 和 $\mathbf{k}^R$。

**为什么不在训练时也走 MQA 模式？** 训练需要每头独立的梯度流来充分训练 $\mathbf{W}^{UK}$ 和 $\mathbf{W}^{UV}$。MQA 模式把所有头压缩为一组共享 KV，虽然推理效率高，但训练时会导致梯度信息过于粗糙。MHA 模式让每头独立解压，确保了压缩-解压的质量。
...
def mla_forward_mqa_mode(h_t, past_c_kv, past_k_r, params):
    ...
    # ════ MQA 模式: c^KV 直接作为共享 KV entry ════
    # 所有头共享同一组压缩表示 + 位置 Key
    t = K_c_kv.shape[0]
    o_list = []
    for i in range(n_heads):
        # Key: 内容(c^KV) + 位置(k^R) — 所有头共享
        K_i = torch.cat([K_c_kv, K_r], dim=-1)     # [t, 576]  ← d_c+d_h^R
        scores = (q[i] @ K_i.T) / ((d_h + d_h_R) ** 0.5)  # [t], d_h+d_h^R=192
        attn_w = F.softmax(scores, dim=-1)          # [t]

        # Value: 从头 i 的上投影矩阵解压
        V_i = params['W_UV_i'] @ K_c_kv.T           # [d_h, t]  ← c^KV → v_i^C
...
> **1M 序列全模型（60 层）KV Cache 对比**：
> - MHA：$28 \text{ GB} \times 60 = 1680 \text{ GB}$（需要 21 张 H100 80GB）
> - MLA：$1.13 \text{ GB} \times 60 = 67.8 \text{ GB}$（勉强放入 1 张 H100 80GB）
> - CSA/HCA 混合：$0.75 \text{ GB} \times 60 = 45 \text{ GB}$（舒适放入 1 张 H100 80GB）

**1M 上下文单 token 推理 FLOPs 对比**（以 V3.2 为 100% 基准）：

| DeepSeek-V3.2 | DeepSeek-V4-Pro | DeepSeek-V4-Flash |
|:------------:|:-------------:|:----------------:|
| 100%（基准） | **27%** | **10%** |

V4-Pro 激活参数是 V3.2 的近 2 倍（49B vs ~37B），但得益于 CSA/HCA 混合架构 + FP4 专家，单 token 推理成本反而降至 27%。
...
$$
\mathbf{q}_i = \mathbf{W}_i^Q \mathbf{h},\quad
\mathbf{k}_i = \mathbf{W}_i^K \mathbf{h},\quad
\mathbf{v}_i = \mathbf{W}_i^V \mathbf{h}
$$
$$
\mathbf{o}_i = \text{Softmax}\!\left(\mathbf{q}_i \mathbf{k}_i^\top / \sqrt{d_h}\right) \mathbf{v}_i,\quad
\mathbf{u} = \mathbf{W}^O [\mathbf{o}_1; \ldots; \mathbf{o}_{n_h}]
$$
$$
\text{KV Cache} = n_h \times d_h \times 2\;\text{元素/token}
$$
...
$$
\mathbf{c}^{KV} = \mathbf{W}^{DKV}\mathbf{h} \quad\text{(KV 联合压缩: $d \to d_c$)}
$$
$$
\mathbf{k}^C = \mathbf{W}^{UK}\mathbf{c}^{KV} \quad\text{(Key 解压)}\qquad
\mathbf{v}^C = \mathbf{W}^{UV}\mathbf{c}^{KV} \quad\text{(Value 解压)}
$$
$$
\mathbf{k}^R = \text{RoPE}(\mathbf{W}^{KR}\mathbf{h}) \quad\text{(解耦位置 Key)}
$$
$$
\mathbf{k}_i = [\mathbf{k}_i^C; \mathbf{k}^R],\quad
\mathbf{q}_i = [\mathbf{W}_i^{UQ}\mathbf{c}^Q; \text{RoPE}(\mathbf{W}_i^{QR}\mathbf{c}^Q)]
$$
$$
\text{KV Cache} = (d_c + d_h^R)\;\text{个元素/token（仅 }\mathbf{c}^{KV}\text{ 和 }\mathbf{k}^R\text{）}
$$
...
$$
I_{t,s} = \sum w^I \cdot \text{ReLU}(\mathbf{q}^I \cdot \mathbf{k}^I) \quad\text{(Lightning Indexer 打分)}
$$
$$
\mathcal{S}_t = \text{Top-k}(\{I_{t,s}\}) \quad\text{(稀疏选择 $k$ 个 token)}
$$
$$
\mathbf{u}_t = \text{SparseAttn}\!\left(\mathbf{h}_t, \{\mathbf{c}_s^{KV}\}_{s \in \mathcal{S}_t}\right)
$$
$$
\text{注意力复杂度: } O(L^2 \to L \cdot k)
$$

$\downarrow$

$\qquad\qquad\qquad$ **CSA（V4，中间层）** $\qquad$ **HCA（V4，浅层/深层）**

$\qquad\check{\mathbf{c}}_m = \text{Pool}(\{\mathbf{c}_s^{KV}\}, r)$ $\qquad$ 多级池化 + 注意力池化

$\qquad\tilde{I}_{t,m} = \text{Indexer}(\check{\mathbf{c}}_m)$ $\qquad$ $\{\check{\mathbf{c}}\} = \text{粗摘要} \oplus \text{细块表示}$

$\qquad$选 Top-k 块 $\to$ 映射回原始$\qquad$ $\mathbf{u}_t = \text{DenseAttn}(\text{所有压缩表示})$

$\qquad\to$ 细粒度稀疏 MLA

$\qquad$保精度 + 计算高效 $\qquad\qquad$ 全覆盖 + 无信息遗漏

### 9.4 设计哲学

DeepSeek 系列的注意力演化遵循一条清晰主线：
...
最终结果：**在 100 万 token 上下文下，V4-Pro 的推理 FLOPs 仅为 V3.2 的 27%，KV cache 仅为 10%**，使百万级长上下文推理成为可实际部署的技术。
