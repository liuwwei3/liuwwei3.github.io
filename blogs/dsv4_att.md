...
**两种模式在数学上严格等价**（详见 [MLA 双模式深度专题](mla_modes_deep_dive) 第 5.3 节的代数证明）。实际推理中可观测的微小差异（< 0.1%）纯粹来自 BF16 浮点运算中求和顺序不同导致的舍入误差。
...
**推理时（MQA Absorb 模式）**：$\mathbf{W}^{UK}$ 预先乘到 Query 侧、$\mathbf{W}^{UV}$ 延迟到加权和之后。**这不是近似——矩阵吸收保证了与 MHA 模式严格等价**（证明见 [MLA 双模式深度专题](mla_modes_deep_dive) 第 5.3 节）。
...
以下代码展示 MLA 推理（MQA naive 模式）的核心逻辑。注意：**实际部署的 DeepSeek-V3 使用更高效的 Absorb 模式**（矩阵吸收），但其逻辑较复杂；下面的代码展示的是等效但更直观的"缓存压缩 KV + 逐头解压 V"方案。两种方案的数学等价性和完整 Absorb 代码见 [MLA 双模式深度专题](mla_modes_deep_dive)。
