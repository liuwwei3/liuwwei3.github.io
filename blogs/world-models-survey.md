...
**Phase 1 的监督来源。** 世界模型的训练是一个标准的有监督+自监督过程。对于从 replay buffer 采样的一段轨迹 $\{(\mathbf{x}_1, \mathbf{a}_1, r_1), ..., (\mathbf{x}_L, \mathbf{a}_L, r_L)\}$，RSSM 首先用编码器和序列模型沿时间轴展开，每一步产生后验状态 $\mathbf{s}_t = \{\mathbf{h}_t, \mathbf{z}_t\}$。然后三个监督信号同时计算：
- **KL 损失**让动态预测器的先验 $\hat{\mathbf{z}}_t$ 逼近编码器的后验 $\mathbf{z}_t$——这是自监督的，因为"标签" $\mathbf{z}_t$ 来自同一条轨迹的编码器输出。
- **重建损失**让解码器从 $\mathbf{s}_t$ 中重建原始观测 $\mathbf{x}_t$——监督信号是原始像素。
- **奖励/终止预测损失**让预测器输出 $\hat{r}_t$ 和 $\hat{c}_t$ 逼近真实的 $r_t$ 和 $\mathbb{1}[\text{episode ended}]$——这是有监督的，因为标签来自环境。
