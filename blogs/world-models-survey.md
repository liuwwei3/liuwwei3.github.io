...
>
> 覆盖 2018–2026，22 篇核心论文 + 1 篇番外，6 个主线节点
>
> 撰写于 2026 年 6 月
...
#### 6.5.5 尾声：LeWorldModel — JEPA 的极简主义实现 (2026.03)

2025 年 11 月，LeCun 离开 Meta，联合创立了 AMI Labs。短短四个月后，他的团队在 arXiv 上发布了 *"LeWorldModel: Stable End-to-End Joint-Embedding Predictive Architecture from Pixels"*（Maes, Le Lidec, LeCun et al., 2026）。这不是又一篇概念性论文——这是第一篇实现了 LeCun 从 2022 年就一直在倡导的**端到端 JEPA 世界模型**的工作。它是 §3 中那张蓝图的最终落地，也是 §4 中 I-JEPA/V-JEPA 的"训练一个编码器做下游任务"路线的完成形态。

**在 §6.5 的合流叙事中，LeWM 居于一个独特的位置。** Dyna 解决了"策略应该在真实和想象上混合训练"的问题。NNT 解决了"表征必须是平稳的"问题。Cosmos 解决了"物理不能仅从数据中学到"的问题。LeWM 解决了**一个更根本的问题**：JEPA 之前从未真正"端到端工作"过。

回顾 §3.3 的讨论：JEPA 面临表征崩塌——编码器和预测器联合训练时，梯度下降贪婪地寻找零损失的平凡解。此前所有的"解决"都是脆弱的启发式。I-JEPA (§4.1) 用 EMA 目标编码器 + VICReg（三个超参数）。DreamerV3 用 KL 平衡 + 自由比特。DINO-WM 直接冻结一个预训练 ViT，放弃端到端学习。PLDM（此前唯一的端到端替代方案）需要 6 个可调损失超参数来维持训练稳定。每一个方案都在增加复杂度来对抗崩塌——但崩塌的根源没有被直接解决。

**LeWM 只用了两个损失项。** (1) 下一个嵌入预测的 MSE 损失 $\|\hat{\mathbf{z}}_{t+1} - \mathbf{z}_{t+1}\|^2$，和 (2) 一个名为 **SIGReg** 的正则化项。SIGReg 的核心思想简单而有力：强制潜表征服从**各向同性高斯分布** $\mathbf{z}_t \sim \mathcal{N}(\mathbf{0}, \mathbf{I})$。在每个训练 batch 中，SIGReg 将潜向量沿着多个随机方向做一维投影，对每个投影做正态性检验，聚合所有方向的统计量，鼓励完整的潜分布匹配高斯。

为什么高斯约束防止崩塌？因为如果编码器输出全零，SIGReg 会极大惩罚它——全零不是一个零均值单位方差的高斯分布。SIGReg 给编码器一个"存活约束"：你的输出**必须是**一个充满多样性的高斯，不能退化。这与 VICReg（§3.3）的维度-对比策略殊途同归——但 SIGReg 只需要**一个**超参数（投影方向数 $D$），而 VICReg 需要三个（方差阈值、协方差权重、不变性权重）。
...
graph LR
    ...
    subgraph Reg["SIGReg 正则化（仅 1 超参数）"]
        Zt --> Proj["随机方向投影"] --> Norm["正态性检验"] --> SIGRegLoss["SIGReg Loss"]
    end
```

**"6 → 1"的极简主义。** PLDM 有 6 个可调损失超参数。LeWM 将其降至 **1**——只需调整投影方向数 $D$。没有任何 EMA，没有任何 stop-gradient，没有任何预训练视觉 backbone，没有任何像素重建，没有任何奖励信号。1500 万参数，一块 GPU 几小时训练完。

**关键结果。** 在 PushT 任务上，LeWM 的完整规划耗时 0.98s——而基于冻结 ViT 的 DINO-WM 需要 47s——**LeWM 快 48×**。这个数字背后的原因是深层且通用的：DINO-WM 的每一次想象 rollout 都要做完整的 ViT 前向传播（~300M 参数），LeWM 的紧凑编码器（15M 参数）让想象近乎实时。在物理探测任务（OGBench-Cube）中，LeWM 的潜空间对方块位置的预测 RMSE 为 **0.002**（MLP probe），DINO-WM 为 0.007——尽管 LeWM 的参数不到其 1/20。

**违反预期实验——世界模型真的"懂"物理吗？** 作者设计了一个巧妙的"违反预期"（Violation of Expectation, VoE）框架。在 PushT 任务中生成三种轨迹：(1) 正常——机器人推方块；(2) 视觉扰动——方块**颜色**突然改变（违反视觉连续性但不违反物理）；(3) 物理扰动——方块被**瞬间传送到随机位置**（违反物理连续性）。一个好的世界模型应该对 (2) 反应轻微，对 (3) 反应剧烈。LeWM 的潜空间预测误差在物理扰动下的峰值显著高于视觉扰动下的峰值——而 PLDM 和 DINO-WM 对两者的响应差异较小。这表明 LeWM 的紧凑潜空间编码了**有意义的物理结构**，而非表面视觉统计。

**一个可证明的防崩塌保证。** 2026 年 5 月，LeCun 的团队发表了理论后续——*"When Does LeJEPA Learn a World Model?"*（Klindt, LeCun, Balestriero, 2026）。这篇论文证明了：当潜变量服从高斯分布、训练数据来自广泛且各向同性的探索时，LeJEPA 的编码器**线性地恢复真实底层变量**（位置、速度、方向）——即学到了一个线性可辨识的世界模型。证明在 **Lean 4 证明助手**中形式化验证。一个重要的条件限制是：有目标导向的训练数据（如只在部分区域探索）会无声地破坏可辨识性保证——为未来的数据收集策略提供了明确指引。

> **为什么 LeWM 是合流叙事中不可或缺的最后一环？** 回到本文的主线——"一个 AI 系统如何学会预测自身行为的后果"。LeWM 给出了从 2018 年到 2026 年这条线的最清晰答案：**不需要重建像素，不需要预训练编码器，不需要多目标损失，不需要 EMA，不需要 stop-gradient。只需要一个编码器、一个预测器、一个预测损失、和一个强制表征多样性的高斯约束。** 极简主义的胜利也是一种哲学胜利：理解世界不需要复杂的机器——它需要的只是一个对表征多样性的正确约束。
...
## 番外：D4RT — 按需查询的 4D 世界重建 (2025.12)

> LeWM 作为 JEPA 路线的最终落地已融入正文 §6.5.5。本篇番外介绍另一股来自计算机视觉的世界建模力量——当 Google DeepMind 用前馈查询取代密集重建，4D 世界建模的效率范式被彻底改写。
...
**为什么 D4RT 对世界模型主线有启发？** 虽然 D4RT 是一个 3D 视觉方法而非 RL 世界模型，但它的范式——"编码一次，按需查询"——与 §3–§4 中 JEPA 的"只预测表征，不重建像素"在哲学上高度共鸣。JEPA 说"不要预测所有像素，只预测需要的表征"；D4RT 说"不要重建所有点，只回答你问的问题"。两者都源于同一个设计原则：**计算应该集中在被需要的信息上，而非被穷举在可能的输出上。** 如果未来有一个"动作条件的"D4RT——不仅回答"这个点在 3D 中在哪"，还能回答"如果自动驾驶车辆做了动作 X，这个点在 3 秒后会在哪"——那它就会成为一个真正的世界模型。正如 §6.5.5 中 LeWM 用"编码 + 高斯约束"完成了 JEPA 路线的极简闭环，D4RT 用"编码 + 点查询"在 4D 视觉中完成了同一种范式转换——两者的交点在于：语言模型有"下一个 token 预测"作为统一接口；世界模型需要的可能正是"下一个查询回答"，而 D4RT 的接口已经准备好了一半。
