...
## 7. 极简主义的胜利：LeWorldModel (2026)

### 7.0 问题引入

回到本文的 §3。LeCun 在 2022 年提出了 JEPA——在表征空间中预测的方法论。接下来的四年里，这条路线经历了两个阶段：

- **概念验证阶段**（§4）：I-JEPA 和 V-JEPA 证明表征空间预测确实比像素重建更高效（10× 快于 MAE、+9.6% 深度预测）。但它们只是**特征提取器**——训练一个编码器用于下游任务，预测器在训练后丢弃。它们没有"想象 rollout"的能力（像 DreamerV3 那样在潜空间中展开多步未来）；它们无法做基于模型的规划（像 Dyna 那样在想象轨迹上训练策略）；它们甚至不是真正端到端的——I-JEPA 依赖 EMA 目标编码器 + VICReg 三个超参数来防止崩塌。

- **启发式修补阶段**（§6.5 合流）：Dyna 解决了"纯想象不如真假混合"的问题（+11.43 pp）。NNT 解决了"表征必须平稳"的问题（+21.60 pp）。但 JEPA 本身的**端到端训练**——即编码器和预测器从随机初始化开始、仅通过预测损失和正则化联合训练、不使用任何预训练组件或冻结权重——始终是一个未解决的挑战。

为什么这么难？答案在 §3.3 中已经给出了：**表征崩塌。** 编码器和预测器联合训练时，梯度下降贪婪地寻找零损失的平凡解——把一切映射为零向量。四年来，每一个"解决方案"都是以增加复杂度为代价来对抗这个根本问题：I-JEPA 用了 EMA + VICReg + 窄预测器（三个超参数、两个独立的编码器架构）。DreamerV3 用了 KL 平衡 + 自由比特（两个额外的 β 系数）。DINO-WM 直接冻结预训练 ViT，放弃端到端学习。PLDM 用 6 个损失项来维持训练稳定——但没有任何理论保证为什么不崩塌。

**LeWorldModel（LeWM）改变了这一切。** 这篇 2026 年 3 月发表在 arXiv 上的 54 页论文（Maes, Le Lidec, LeCun et al.），来自 LeCun 新创立的 AMI Labs，实现了第一个**真正端到端、从随机初始化像素出发、只用两个损失项稳定训练**的 JEPA 世界模型。它不仅是 §3 中那张蓝图的最终落地——它是 JEPA 路线从 2022 年概念提出到 2026 年工程闭环的完成形态。

### 7.1 背景：那些未能成功的 JEPA 实现

在深入 LeWM 之前，有必要理解此前 JEPA 实践者在"端到端"道路上的挣扎——因为 LeWM 的极简主义只有与这些努力放在一起对比，才能看出它有多么反直觉。

**PLDM（Predictive Latent Dynamics Model, Nguyen et al., 2024）** 是 LeWM 之前唯一的端到端 JEPA 替代方案。它不依赖预训练编码器、不依赖 EMA、不依赖 stop-gradient——这三个"不依赖"听起来正是 LeWM 所追求的。但代价是：**6 个独立可调的损失超参数。** PLDM 的训练目标是一个精心设计的多目标优化问题——编码器必须在"保留视觉信息"（重建损失）、"使表征可预测"（预测损失）、"防止崩塌"（某种正则化项）、"控制表征范数"、"平滑潜空间"、和"保持动作相关性"这六个有时相互冲突的目标之间找到平衡。在实践中，这 6 个超参数之间的交互极其复杂——调整一个系数可能会使另一个系数的有效范围变化一个数量级。PLDM 在论文中报告了一些任务的实验结果，但它从未像 DreamerV3 那样在 150+ 个任务上展示泛化性。

**DINO-WM（DINO-based World Model, Bousselham et al., 2024）** 走了一条相反的路——用质量换简单。它冻结了一个在大规模图像数据上预训练的 ViT（具体是 DINOv2 的 300M+ 参数 backbone），然后在冻结的特征上用 MLP 学习潜动态。这样就没有崩塌问题——编码器已经被冻结了——但代价是沉重的：失去了端到端学习的能力、推理时的每一次前向传播都要消费完整的 ViT、规划速度受限于冻结 backbone 的推理延迟、模型的行为被限制在预训练知识的范围内（如果预训练数据中没有见过某种物理交互，DINO-WM 的编码器无法学到对应的表征）。

**DreamerV3（§2.1）** 虽然主要是 RSSM 路线，但它对 JEPA 问题也有贡献。它的 RSSM 通过 KL 平衡（$\beta_{\text{dyn}}=0.5, \beta_{\text{rep}}=0.1$）让更多信息通过确定性路径流动，同时用自由比特（KL 低于 1 nat 时停止惩罚）防止编码器崩塌。但 RSSM 仍然依赖像素解码器——它在 §2.1 中的表现被"编码无关细节"的包袱拖累。

**I-JEPA/V-JEPA（§4.1–§4.2）** 是表征空间预测的首次大规模实证验证——但它们不是世界模型。它们的目标是训练一个通用的视觉编码器用于下游任务（分类、检测、分割），而不是一个能做想象 rollout 和规划的潜动态模型。预测器在训练后丢弃，编码器不接收动作条件——它们缺少世界模型最核心的能力：回答"如果我做 X，接下来会发生什么"。

这些努力的共同点是：它们都不约而同地接受了"需要在简单性和崩塌防护之间做权衡"这个前提。PLDM 用 6 个超参数换来了端到端；DINO-WM 用"放弃端到端"换来了简单；DreamerV3 用像素解码器 + KL 平衡换来了跨任务泛化。LeWM 说：**不需要权衡。只有一个约束就足够了。**

### 7.2 LeWM 的核心架构：两个损失，一个约束

LeWM 的架构设计哲学是一个反直觉的洞察：**防崩塌的关键不是"更多的正则化"，而是"正确的正则化"。** 整个系统只包含三个组件和一个正则化项：
...
**编码器。** 一个轻量 CNN/ViT，将原始像素 $\mathbf{o}_t$ 映射为潜向量 $\mathbf{z}_t \in \mathbb{R}^n$（$n$ 通常是 64–256）。两个时间步的编码器**权重共享**——不需要 EMA 副本。不需要在训练期间冻结任何权重。

**预测器。** 一个标准 MLP，接收 $(\mathbf{z}_t, \mathbf{a}_t)$，输出 $\hat{\mathbf{z}}_{t+1}$——对下一个潜表征的预测。预测损失是平淡无奇的均方误差：$\mathcal{L}_{\text{pred}} = \|\hat{\mathbf{z}}_{t+1} - \mathbf{z}_{t+1}\|^2$。这里没有 stop-gradient——编码器的参数 $\theta_{\text{enc}}$ **同时**被预测损失优化。这正是此前所有 JEPA 实践者避开的做法——因为同时优化编码器和预测器是崩塌的直接原因。

**SIGReg——为什么一个约束就足够了。** SIGReg（**S**tochastic **I**sotropic **G**aussian **Reg**ularization）是 LeWM 的核心创新——也是唯一防止崩塌的机制。它的思想既简单又深刻：

*Step 1：对潜向量做随机一维投影。* 给定一个 batch 的 $B$ 个潜向量 $\mathbf{Z} = \{\mathbf{z}^{(1)}, \mathbf{z}^{(2)}, ..., \mathbf{z}^{(B)}\} \in \mathbb{R}^{B \times n}$，生成 $D$ 个随机方向向量 $\mathbf{w}_1, \mathbf{w}_2, ..., \mathbf{w}_D \in \mathbb{R}^n$（通常 $D=256$）。对每个方向 $\mathbf{w}_d$，将整个 batch 投影到该方向：$\mathbf{p}_d = \mathbf{Z} \mathbf{w}_d \in \mathbb{R}^B$。每个投影是一个长度为 $B$ 的一维分布。

*Step 2：在每个投影方向上进行正态性检验。* 对于投影 $\mathbf{p}_d$，计算其偏度（样本三阶矩）和峰度（样本四阶矩），并与理论标准高斯的值（偏度=0, 峰度=3）进行比较：
...
其中 $\bar{p}_d$ 和 $\sigma_d$ 分别是投影的样本均值和样本标准差。SIGReg 的损失为：

$$\mathcal{L}_{\text{SIGReg}} = \frac{1}{D}\sum_{d=1}^{D} \Big(\text{skew}_d^2 + (\text{kurt}_d - 3)^2\Big)$$

*Step 3：聚合所有投影方向的统计量。* 总损失是 $\mathcal{L} = \mathcal{L}_{\text{pred}} + \lambda \cdot \mathcal{L}_{\text{SIGReg}}$，其中 $\lambda=1$ 通常就足够。

**为什么这防止了崩塌？** 如果编码器输出全零，那么所有投影都是零——均值为 0、标准差为 0，偏度无定义、峰度也无定义，SIGReg 惩罚极大。更重要的是，即使编码器找到一个不那么平凡的崩塌点（如所有输出都沿着某个固定的低维流形），SIGRec 的随机投影也能检测到——因为总存在一个投影方向使得投影分布显著偏离标准高斯。换句话说，SIGReg 要求编码器的输出在**所有方向上**都是高斯的——这阻止了表征在任何一个方向上退化。

**直觉。** VICReg（§3.3）通过约束维度间相关性来防止表征退化——"不同的维度不应该互相相关"。SIGReg 更进一步——它说"每个随机方向的投影都应该看起来像高斯噪声"。这是一个更强的约束——如果一个分布在所有一维投影上都匹配高斯，则它本身就是各向同性高斯（Cramér-Wold 定理的变体）。**VICReg 说"维度之间要多样"；SIGReg 说"像一个标准高斯那样多样"。**

**与 VICReg 的关键对比：**

| 维度 | VICReg（I-JEPA） | SIGReg（LeWM） |
|------|-----------------|----------------|
| 超参数数量 | 3（方差阈值 γ, 协方差权重 ν, 不变性权重 μ） | **1（投影方向数 D）** |
| 约束对象 | 维度之间的相关性矩阵 | 随机方向上的分布矩 |
| 数学原理 | 每对维度去相关 → 表征空间充分展开 | 所有一维投影匹配高斯 → 完整分布匹配各向同性高斯 |
| EMA 需求 | 需要（防止 Enc_c 和 Enc_t 合谋） | **不需要**（SIGReg 直接约束编码器输出） |
| stop-gradient | 需要（目标编码器不接收梯度） | **不需要**（编码器能安全地接收来自预测损失的梯度） |

> **▸ 小专题：为什么 SIGReg 允许去掉 EMA 和 stop-gradient？**
>
> I-JEPA 中 EMA 和 stop-gradient 的作用是**隔离**两个编码器——如果 $\text{Enc}_c$ 和 $\text{Enc}_t$ 都接收梯度，它们会合谋输出相同的平凡表征。但在 LeWM 中，编码器是**单一网络**（两个时间步共享权重），不存在两个编码器合谋的问题。唯一的危险是编码器自身在预测损失和 SIGReg 的张力下崩塌——而 SIGReg 直接约束了编码器的输出分布，使得崩塌在数学上是不可能的。
>
> 具体来说：如果编码器试图输出全零来使预测损失为零，SIGRec 会在所有投影方向上检测到非高斯性（全零没有方差），给出巨大的惩罚。如果编码器试图输出一个固定的非零向量（所有输入映射到同一个值），SIGRec 仍然会检测到——因为固定向量的投影是一个 Dirac delta，不是高斯。编码器唯一能同时满足"可预测"（预测损失小）和"高斯"（SIGReg 小）的方式是：**为不同的观测产生不同的、但结构上相似的潜表征——这正是我们想要的。**

### 7.3 关键结果

| 指标 | LeWM | PLDM (6 超参数) | DINO-WM (冻结 ViT) | DreamerV3 |
|------|------|------------------|---------------------|-----------|
| **模型参数** | ~15M | ~20M | ~300M+ | ~20–200M |
| **损失超参数** | **1** | 6 | 0（编码器冻结） | 多个（KL β, 自由比特...） |
| **训练速度** | 单 GPU，几小时 | 单 GPU | —（编码器冻结） | 多 GPU，35 小时 |
| **规划速度**（PushT） | **0.98s** | — | 47s | — |
| **物理探测**（方块位置 RMSE） | **0.002** | 0.003 | 0.007 | — |
| **VoE 物理 vs 视觉扰动分离** | **显著分离** | 弱分离 | 弱分离 | — |
| **防崩塌保证** | **Lean 4 形式化证明** | 无 | 无（编码器冻结） | 无 |

**规划速度 48× DINO-WM 的原因。** DINO-WM 冻结了一个 DINOv2 ViT (~300M 参数) 作为编码器，每次想象 rollout 都要做完整的 ViT 前向传播。LeWM 的编码器只有 15M 参数——规划时每一"步想象"几乎是指令级的。在 16 步的 MPC 规划中，DINO-WM 的编码器前向传播 16 × 300M 参数 vs LeWM 的 16 × 15M 参数——这不是 48× 差别的全部原因（还有解码器、注意计算等的差异），但编码器大小是最大的单一贡献因子。

### 7.4 理论支撑：LeJEPA 什么时候学到世界模型？

2026 年 5 月，同一团队发表了理论后续——*"When Does LeJEPA Learn a World Model?"*（Klindt, LeCun, Balestriero, 2026）。这篇论文回答了 JEPA 路线的底层理论问题：**在什么条件下，JEPA 的潜表征能线性恢复真实的物理状态变量？**

核心结果：当三个条件满足时，LeJEPA 的编码器 $\phi$ 满足 $\phi(\mathbf{x}_t) = A \cdot \mathbf{s}_t^{\text{true}} + \mathbf{b}$（其中 $\mathbf{s}_t^{\text{true}}$ 是真实状态——位置、速度、方向等，$A$ 是一个满秩矩阵）。三个条件是：

1. **潜变量服从高斯分布**：由 SIGReg 保证（这正是正则化选择高斯的深层原因——以便得到可证明的保证）
2. **训练数据来自各向同性探索**：数据必须广泛覆盖状态空间，使模型有机会看到"不同的位置 + 不同的速度"的各种组合
3. **编码器和预测器在联合损失下收敛**：由 LeWM 的双损失训练保证

关键的限制条件：**目标导向的探索会破坏可辨识性。** 如果 agent 只在"通向目标的有用区域"内探索——比如 PushT 中机器人只接触方块附近的空间——模型接收不到足够的多样性来解析位置与速度之间的相关性。在这种条件下，编码器可能将位置和速度混淆（因为它们在有用训练数据中是共线的），而 SIGRec 不会报错——两个高度相关的维度仍然可以各自服从高斯分布。这是一个有实践意义的警告：**收集世界模型的训练数据需要"多余的"探索——数据越广泛，模型学得越好。** 证明在 **Lean 4 证明助手**中形式化验证。

### 7.5 延伸：D4RT 与查询式世界模型

LeWM 代表了 JEPA 路线在 2026 年的集大成。与此同时，一股来自 3D 计算机视觉的力量——Google DeepMind 的 **D4RT**（Zhang, Le Moing et al., 2025.12）——用"编码一次，按需查询"的范式解决了动态 4D 场景重建问题，在方法论的极简主义上与 LeWM 深度共鸣。

D4RT 将整个视频一次性编码为全局场景表征 $\mathbf{F}$，然后任意数量的独立点查询——"源帧 $t_{\text{src}}$ 上 $(u,v)$ 位置的像素，在目标时刻 $t_{\text{tgt}}$ 的 3D 位置是什么？"——通过交叉注意力解码器从 $\mathbf{F}$ 中直接回答。位姿估计 200+ FPS（100× MegaSaM），点轨迹比 SOTA 高 18–300×。它的方法论与 LeWM/JEPA 高度共鸣：**不要穷举重建——只回答你问的问题。** JEPA 说"不要预测所有像素，只预测需要的表征"；D4RT 说"不要重建所有点，只回答你问的问题"。两者的交点在于：语言模型有"下一个 token 预测"作为统一接口；世界模型需要的可能正是"下一个查询回答"——而 D4RT 的接口已经准备好了一半。

> **主线节点 7。** LeWorldModel 完成了从 §3 到 §7 的 JEPA 叙事闭环：一个不需要 EMA、不需要 stop-gradient、不需要预训练编码器、不需要多目标损失的端到端世界模型。只需要一个编码器、一个预测器、一个 MSE 预测损失、和一个 SIGReg 高斯正则化——1500 万参数、1 个超参数、单 GPU 训练、48× 规划加速。SIGRec 背后的数学保证了编码器的输出在所有方向上都是高斯的——对表征多样性的正确约束被证明比模型容量或训练技巧都更重要。D4RT 在 3D 视觉中的"编码一次，按需查询"与 LeWM 的"编码一次，预测未来"形成了方法论上的交叉印证。**世界模型的极简时代，可能刚刚开始。**
