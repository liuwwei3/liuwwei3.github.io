...
>
> 覆盖 2018–2026，20 篇核心论文 + 2 篇番外，6 个主线节点
>
> 撰写于 2026 年 6 月
...
## 番外：两颗新星——LeWM 与 D4RT (2026)

> 本文主体覆盖 2018–2026 年初。2025 年底至 2026 年中，两篇性质不同但同样重要的论文出现在 arXiv 上，为理解"什么是世界模型的正确形态"提供了新的视角。一篇来自 LeCun 新创立的 AMI Labs，将 JEPA 的想法推到极简主义极致；另一篇来自 Google DeepMind，用前馈查询取代密集重建，为 4D 世界建模开辟了新的效率范式。

### 番外 1：LeWorldModel — JEPA 的极简主义实现 (2026.03)

2025 年 11 月，LeCun 离开 Meta，联合创立了 AMI Labs。短短四个月后，他的团队在 arXiv 上发布了 *"LeWorldModel: Stable End-to-End Joint-Embedding Predictive Architecture from Pixels"*（Maes, Le Lidec, LeCun et al., 2026）——这是第一篇实现了 LeCun 从 2022 年就一直在倡导的端到端 JEPA 世界模型的论文。

**为什么之前没有人做到？** 回顾 §3.3 的讨论：JEPA 面临表征崩塌——编码器和预测器联合训练时，梯度下降贪婪地寻找零损失的平凡解（所有表征 = 0，所有预测 = 0）。此前的解决手段包括 EMA 目标编码器（I-JEPA, V-JEPA）、多目标损失函数（DreamerV3 的 KL 平衡）、或冻结预训练编码器（DINO-WM）。每一项都是脆弱的启发式——增加超参数、引入训练不稳定、或牺牲端到端学习的能力。

**LeWM 的解决方案——SIGReg。** LeWM 只用了**两个损失项**：(1) 下一个嵌入预测的 MSE 损失，和 (2) 一个名为 **SIGReg** 的正则化项，强制潜表征服从各向同性高斯分布 $\mathbf{z}_t \sim \mathcal{N}(\mathbf{0}, \mathbf{I})$。SIGReg 的数学直觉简单而有力：

1. 在每个训练 batch 中，将潜向量 $\mathbf{z}_t$ 沿着多个**随机方向**做一维投影
2. 对每个投影方向，做正态性检验（计算投影分布与标准高斯的距离）
3. 聚合所有方向的统计量，鼓励完整的潜分布匹配高斯

为什么高斯约束防止崩塌？因为如果编码器输出全零，SIGReg 会极大惩罚它（全零不是一个零均值单位方差的高斯分布）。SIGReg 本质上是给编码器一个"存活约束"——你的输出**必须是**一个充满多样性的高斯分布，不能退化。这与 VICReg（§3.3）的维度-对比策略殊途同归，但 SIGReg 只需要**一个超参数**（投影方向的采样数量），而 VICReg 需要三个（方差阈值、协方差权重、不变性权重）。

**"6 → 1"的极简主义。** PLDM（此前唯一的端到端 JEPA 替代方案）有 6 个可调损失超参数。LeWM 将这一数字降至 **1**——只需要调整投影方向数 $D$。这使得超参数搜索可以在对数时间（logarithmic-time）内完成——无需在 6 维网格中漫无目的地搜索。
...
graph LR
    subgraph Input["观测序列"]
        O["o_t<br/>当前帧"] --> Enc["Encoder<br/>CNN / ViT"]
        O_next["o_{t+1}<br/>下一帧"] --> Enc_next["Encoder<br/>（权重共享）"]
    end
    ...
    subgraph Latent["潜空间"]
        ...
        Act["a_t"] --> Pred
        Pred --> Zt1_hat["ẑ_{t+1}"]
        Zt1_hat --> MSE["MSE Loss<br/>‖ẑ_{t+1} − z_{t+1}‖²"]
    end
    
    subgraph Regularization["SIGReg 正则化"]
        Zt --> Proj["随机方向投影"] --> Norm["正态性检验"] --> Loss2["SIGReg Loss"]
    end
    
    style Enc fill:#e8f5e9
    style Pred fill:#fff3e0
    style SIGReg fill:#e3f2fd
```

与 I-JEPA/V-JEPA 相比，LeWM 没有 EMA 目标编码器，没有 stop-gradient，没有预训练视觉 backbone，没有像素重建——纯端到端，1500 万参数，一块 GPU 几个小时就能训练完。

**关键结果：**

| 指标 | LeWM | 对比 |
|------|------|------|
| **模型参数** | ~15M | DINO-WM: 冻结 ViT backbone（~300M+） |
| **损失超参数** | **1**（投影方向数 D） | PLDM: 6, DreamerV3: 多个（KL 系数, 自由比特阈值...） |
| **训练速度** | 单 GPU，几小时 | DreamerV3: 多 GPU，35 小时 |
| **规划速度** | 0.98s（PushT 完整规划） | DINO-WM: 47s——**LeWM 快 48×** |
| **物理探测**（方块位置 RMSE） | **0.002**（MLP probe） | DINO-WM: 0.007, PLDM: 0.003 |
| **端到端执行成功率**（PushT） | 可与 DINO-WM 匹敌 | 在固定 FLOPs 预算下，LeWM 远超 DINO-WM |

表中最引人注目的是第 4 行：**规划速度快 48×。** DINO-WM 的沉重代价来自它冻结的大型预训练 ViT——每次规划 rollout 都要做完整的 ViT 前向传播。LeWM 的紧凑编码器（仅 15M 参数）让想象 rollout 近乎实时。

**违反预期实验——世界模型真的"懂"物理吗？** LeWM 的作者设计了一个巧妙的"违反预期"（Violation of Expectation, VoE）框架来测试世界模型是否真正理解了物理——而非仅仅记住了训练数据的统计模式。在 PushT 任务中，他们生成三种轨迹：

1. **正常轨迹**：机器人正常推方块，方块平稳移动
2. **视觉扰动**：方块的**颜色**突然改变——这违反视觉连续性，但不违反物理
3. **物理扰动**：机器人和方块被**瞬间传送到随机位置**——这违反物理连续性（物体不能瞬间消失并出现在别处）

一个好的世界模型应该对 (2) 反应轻微（"哦，颜色变了"），对 (3) 反应剧烈（"这不合理！物体不能瞬间传送！"）。LeWM 的潜空间预测误差在物理扰动下的峰值显著高于视觉扰动下的峰值——而 PLDM 和 DINO-WM 对两者的响应差异较小。这表明 LeWM 的紧凑潜空间编码了**有意义的物理结构**，而非表面视觉统计。

**一个可证明的防崩塌保证。** 2026 年 5 月，LeCun 的团队发表了一篇理论后续——*"When Does LeJEPA Learn a World Model?"*（Klindt, LeCun, Balestriero, 2026）。这篇论文证明了：当潜变量服从高斯分布、训练数据来自广泛且各向同性的探索时，LeJEPA 的编码器**线性地恢复真实底层变量**（位置、速度、方向）——即学到了一个线性可辨识的世界模型。更有力的结果是：在潜空间中优化的策略，产生与在真实潜空间中相同的动作。证明在 **Lean 4 证明助手**中形式化验证。同时，他们发现了一个重要的条件限制：有目标导向的训练数据（如只在部分区域内探索）会无声地破坏可辨识性保证——为未来的数据收集策略提供了明确指引。

> **为什么 LeWM 对本文的主线重要？** LeWM 是将 LeCun 2022 年蓝图（本文 §3）在 2026 年完整落地的工作。它回答了一个悬置了四年的问题：**JEPA 不需要 EMA、不需要 stop-gradient、不需要预训练编码器、不需要像素重建——只要给编码器一个高斯约束，它就能稳定地从原始像素学到有物理意义的潜表征。** 极简主义的胜利也是一种哲学胜利：好的世界模型不需要复杂的机器——它需要的只是一个对表征多样性的正确约束。

---

### 番外 2：D4RT — 按需查询的 4D 世界重建 (2025.12)

Google DeepMind 在 2025 年 12 月发布的 D4RT（**D**ynamic **4**D **R**econstruction and **T**racking）提出了一个令人耳目一新的范式转换：**不重建整个世界——只回答你问的问题。**

**传统方法的根本困境。** 标准的 3D/4D 重建方法是"穷举式"的——"把所有像素的 3D 位置都算出来，然后把它们拼成一个模型，然后从这个模型中提取你需要的信息"。但动态场景（本文 §6 中的驾驶、机器人操作、人体运动）带来一个根本挑战：场景在变化，穷举重建的代价随时间呈线性增长——每秒 30 帧就意味着每帧都需要一次完整的重建。

**D4RT 的核心洞见：编码一次，按需查询。** D4RT 的流程只有两步：

1. **编码**：一个大型 ViT 编码器将整个视频一次性编码为一个**全局场景表征** $\mathbf{F} \in \mathbb{R}^{N \times C}$。这是唯一的"重"计算——之后 $\mathbf{F}$ 保持固定。
2. **查询**：一个轻量解码器接受任意数量的独立查询 $q = (u, v, t_{\text{src}}, t_{\text{tgt}}, t_{\text{cam}})$，每个查询指定"源帧 $t_{\text{src}}$ 上的 2D 像素 $(u,v)$ 在目标时刻 $t_{\text{tgt}}$、从相机 $t_{\text{cam}}$ 的视角看，3D 位置在哪？"——输出 $P \in \mathbb{R}^3$。

查询在**全部时间和空间**上被解耦：$t_{\text{src}}$（源时刻）、$t_{\text{tgt}}$（目标时刻）、$t_{\text{cam}}$（参考相机）三个时间索引可以**完全不同**。如果要追踪一个行人的轨迹，你可以在 $t_{\text{src}}=5$ 帧上选定脚踝的像素 $(0.3, 0.8)$，然后查询 $t_{\text{tgt}} = 1, 2, 3, ..., T$ 上该点的 3D 位置——得到完整的 3D 轨迹。如果要重建全场景点云，你可以对所有像素逐一查询——但这是可选的，不是必需的。

```mermaid
graph LR
    subgraph Encode["Stage 1: 全局编码（一次）"]
        Video["输入视频<br/>T×H×W×3"] --> ViT["ViT 编码器<br/>（VideoMAE 初始化）"]
        ViT --> F["全局场景表征 F<br/>N×C"]
    end
    
    subgraph Query["Stage 2: 按需查询（任意次）"]
        F --> Dec["轻量交叉注意力解码器"]
        Q["查询 q = (u, v, t_src, t_tgt, t_cam)"] --> Dec
        Dec --> P["3D 位置 P ∈ ℝ³"]
    end

    subgraph Outputs["下游任务（统一接口）"]
        P --> Track["点轨迹"]
        P --> PC["点云"]
        P --> Depth["深度图"]
        P --> Pose["相机位姿"]
...
**关键性能指标：** D4RT 在 Sintel、ScanNet、KITTI、Bonn 四个数据集上全面超越了此前方法。相机位姿估计（ATE）在 Sintel 上从 MegaSaM 的 0.334 降至 **0.091**——误差减少 73%。更重要的是**速度**：位姿估计超过 200 FPS（A100），是 VGGT 的 9×、MegaSaM 的 100×。跟踪点数量比我此前的 SOTA 方法高出 **18–300×**。

**为什么 D4RT 对世界模型主线有启发？** 虽然 D4RT 是一个 3D 视觉方法而非 RL 世界模型，但它的范式——"编码一次，按需查询"——与 §4 中 JEPA 的"只预测表征，不重建像素"在哲学上高度共鸣。JEPA 说"不要预测所有像素，只预测需要的表征"；D4RT 说"不要重建所有点，只回答你问的问题"。两者都源于同一个设计原则：**计算应该集中在被需要的信息上，而非被穷举在可能的输出上。** 如果未来有一个"动作条件的" D4RT——不仅回答"这个点在 3D 中在哪"，还能回答"如果自动驾驶车辆做了动作 X，这个点在 3 秒后会在哪"——那它就会成为一个真正的世界模型。事实上，这正是 GAIA-1 和 OccWorld（§6.2）在自动驾驶中试图做的，但 D4RT 的点查询接口比密集图像生成优雅得多。

---

### 番外篇的收束

LeWM 和 D4RT 来自两个截然不同的研究传统——RL/控制 vs 计算机视觉——却朝着同一方向收敛：

- **极简主义**：LeWM 将 JEPA 的超参数从 6 个减到 1 个；D4RT 将所有 4D 任务统一到一个查询接口。
- **只计算需要的东西**：LeWM 不在推理时预测未来（除非做 MPC 规划时才需要，回看 §1.3 的 MPC 延伸讨论）；D4RT 不在编码后重建整个场景，只按需解码查询点。
- **单 GPU 可用**：LeWM 的 1500 万参数 + D4RT 的编码一次策略，让前沿世界模型不再需要数据中心级别的资源。

两者的交点或许在字面上已经提到的那句话里："如果 D4RT 能接受**动作**作为查询条件......"那么 4D 重建和世界模型预测之间的那条线就会消失。LeCun 在 AMI Labs 的豪赌与 DeepMind 对 4D 重建效率的极致追求，最终可能汇聚于同一个架构：一个编码器（理解世界），一个查询接口（回答"如果我做 X，这个点会在哪"），和一个高斯约束（防止表征崩塌）。
