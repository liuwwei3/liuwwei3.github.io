...
§3 证明了像素预测的世界模型能走很远——DreamerV3 统治了 150+ 个任务，IRIS 把世界建模重铸为序列预测。但它们始终背负着一个固有的负担：解码器必须重建与任务无关的视觉细节。
...
LeCun 在 §2 中提出了 JEPA 作为替代方案。但直到 2023 年，这只是一个纸面上的假设。**一个在表征空间中预测的模型，真的能在实际任务上打败在像素空间中预测的模型吗？** 换句话说：一个"不做梦"的世界模型——不生成任何图像、只预测抽象表征——能学到有用的知识吗？

I-JEPA 是第一个对这个问题给出大规模实证答案的工作。而 V-JEPA 则将这个答案从静态图像推到了动态视频。

### 4.1 I-JEPA：表征空间预测的实证胜利

Mahmoud Assran 等人（Meta FAIR, CVPR 2023）的 I-JEPA 是 LeCun 世界模型愿景的第一个大规模实现。要理解它的设计，需要回到 §2.2 中 JEPA 的核心公式——但这次，我们有了一个具体的、可运行的架构。
...
**决策 1：遮盖在输出侧，而非输入侧。** 目标编码器 $\text{Enc}_t$ **始终看到完整图像**并产生高层语义表征。遮盖只应用于损失计算——仅被遮盖的区域需要被预测。这与 MAE（He et al., 2022）有根本区别：MAE 在输入侧随机遮盖 75% 的 patch，编码器只看到剩余的 25%；I-JEPA 的目标编码器看到 100% 的图像。结果差异是显著的——MAE 的目标表征是"从残缺输入中挤出来的"，质量被输入遮盖限制；I-JEPA 的目标表征是"从完整上下文中提取的"，语义更丰富。
...
1. 从 ImageNet-1K 中取一张图像，随机选择 4 个目标块（每个覆盖 15-20% 的图像面积，宽高比 0.75-1.5）
2. 上下文块 = 整张图像减去目标块（即上下文块覆盖 85-100% 的图像区域）
...
> **主线节点 4。** I-JEPA 和 V-JEPA 为 LeCun 在 §2 中的假设提供了关键的实证支持：**在表征空间中预测不仅在理论上更优雅，在实践中也更高效。** JEPA 的核心优势——计算效率（10× MAE）、语义理解（+9.6% 深度预测）、时序推理（+6% Something-Something-v2）——都来源于同一个设计选择：抛弃像素解码器，让编码器自由决定"什么是值得保留的"。但 JEPA 的一个局限也由此暴露：**它的表征是抽象的、不可视化的——你无法直接"看到"世界模型预测了什么。** 表征是否"正确"只能通过下游任务间接验证。这个问题在 §5 将被放大：当视频生成模型（如 Sora）声称自己也是世界模型时，整个领域面临一个核心认识论问题——**"理解世界"和"能画出世界"之间，到底是什么关系？**
...
这场辩论对本文的主线至关重要。回顾 LeCun 在 §2 中提出的 JEPA 路线的核心论证：在像素空间中预测是低效的，因为你浪费了表征容量在不可预测的细节上。Sora 似乎用它的涌现能力反驳了这个论点——如果一个纯像素预测模型能学会咬汉堡留牙印，那么 JEPA 的'不要预测像素'就失去了理论基础。**但如果 Sora 的'物理理解'只是表面幻象呢？**
...
**Cosmos-Transfer 2.5。** 世界变换模型——可以执行 Sim2Real（将仿真中的粗糙渲染转化为逼真画质）或 Real2Real（如白天→夜晚、晴天→雨天）。在自动驾驶场景中，用 Transfer 2.5 做数据增强后，3D 车道线和立体框检测的准确率提升高达 60%。
...
§1–§5 的所有世界模型共享一个局限：它们预测**接下来会发生什么**，但它们自己不行动。Ha & Schmidhuber 的世界模型预测 $\mathbf{z}_{t+1}$，但只为了训练 C——推理时 C 只消费 $\mathbf{h}_t$。DreamerV3 在想象中展开 16 步，但也只为了训练 Actor——部署时 Actor 是一个前向传谣。I-JEPA 和 V-JEPA 从来没有被设计为交互式的——它们是被动的表征学习器。

但世界模型的终极意义不是"旁观"，而是**参与**。一个真正的世界模型应该能回答：**"如果我做 X，接下来会发生什么？"** 这里的"如果我做 X"是关键——动作是输入，预测是输出。

2023–2025 年间，三条路线将世界模型从"观察世界"推进到"活在世界中"：

1. **交互式环境生成**——Genie 和 UniSim 将世界模型本身变成一个可互动的生成引擎
2. **自动驾驶世界模型**——GAIA-1、OccWorld 在真实世界的驾驶数据上学习动作条件下的未来预测
3. **物理机器人学习**——DayDreamer 证明世界模型可以直接在真实硬件上在线学习

它们的共同主题是：世界模型必须接受**动作**作为输入，并产生**可供决策使用的反馈**——而不仅仅是好看的预测。

### 6.1 交互式环境生成：Genie 与 UniSim

**Genie 系列（Google DeepMind, 2024–2025）。** Genie 的核心思想是大胆且反直觉的：**从互联网上未标注的游戏视频中学习一个可交互的世界模型——无需任何动作标签。** 想象你在 YouTube 上看了 100 万个 Mario 游戏视频，但所有视频都没有告诉你玩家当时按了什么键。你能从这些纯像素流中学会"怎么玩 Mario"吗？

Genie 的答案是：能。它做到了以下几步：

1. **自动发现动作空间。** 模型通过一个**可学习的动作编码器**（latent action model），从连续的视频帧中自动推断"什么构成了有意义的动作单元"——向前走、跳跃、射击等。在训练阶段，这个潜在动作是从**过去到未来的帧差异**中学习到的——"如果帧间出现了这 8 种有意义的像素变化模式，那这可能就是 8 种基础动作。"

2. **视频分词（Video Tokenizer）。** 类似 VQ-VAE，将视频帧压缩为离散 token。Genie 直接在这个离散空间中做预测。

3. **时空 Transformer。** 用因果 Transformer 建模"过去的帧 token + 过去推断的潜在动作 → 未来的帧 token"。注意力掩码确保模型只能看到过去——这是一个自回归世界模型。

**Genie 的三个迭代版本：**

| 版本 | 发布时间 | 关键特性 | 主要局限 |
|------|---------|---------|---------|
| **Genie 1** | 2024.03 | 110 亿参数，从 2D 平台游戏视频中学到可交互世界，~1 FPS | 仅 2D，极慢 |
| **Genie 2** | 2024.12 | 从单张图片生成可交互 3D 环境，360p，10-60 秒一致性，键盘鼠标交互。技术核心：自回归潜扩散模型 | 交互时长短，远未达实时 |
| **Genie 3** | 2025.08 | 720p@24fps，数分钟一致性，实时交互，可提示世界事件。Waymo 基于 Genie 3 创建了驾驶仿真世界模型 | 高速动态场景仍有退化 |

**Genie 2 的技术核心——自回归潜扩散模型。** 在每一个时间步，模型做以下操作：
1. 自动编码器将当前帧和未来帧压缩为"潜帧"（latent frames）
2. Transformer 动态模型通过因果掩码学习帧间关系——"给定过去的潜帧和潜在动作，预测下一帧的潜帧"
3. 自回归采样逐帧生成——与 LLM 逐 token 生成完全平行的逻辑，只是 tokens 换成了潜帧
4. 无分类器引导（classifier-free guidance）提高动作可控性——在训练时随机丢弃动作条件，推理时用"有动作条件 - 无动作条件"的残差来增强动作的响应性

**为什么 Genie 是一个突破？** 传统的游戏 AI 需要手工标注的动作数据（"在第 2437 帧，玩家按了右键"）。Genie 证明了这种标注不是必须的——**像素流本身包含足够的信息来推断动作空间的结构**。这是一个"世界模型可以从被动观察中学习"的重要原则性证明。

**但它的核心弱点是自回归误差累积。** 10-20 秒后，场景开始退化为模糊的"梦境般"纹理——这是自回归生成的根本局限。每一帧的微小预测误差（潜空间中差 0.01）在 100 帧后被放大到完全脱离真实潜空间，模型进入了训练数据从未覆盖的"幻觉区域"。这引出了 §7 的核心问题：如何打破自回归的误差累积魔咒？

**UniSim（UC Berkeley / DeepMind, 2023，ICLR 2024 杰出论文）。** 如果说 Genie 解决了"无标注视频 → 交互世界"的问题，UniSim 解决的是一个不同的问题：**如何将来自完全不同来源的数据，编织成同一个世界模型的训练信号？**

UniSim 的训练数据源之广令人印象深刻：Habitat 模拟器的室内导航视频、Bridge 真实机器人操作数据、Ego4D 人类自我中心视频、Matterport3D 全景扫描、LAION 互联网图文对。**如果直接混在一起，模型会看到"机器人的夹爪在抓罐子"紧接着"一个人在做早餐"紧接着"一个空房间的 360° 全景"——对任何时序预测模型来说都是灾难。**

UniSim 的关键创新——**统一动作空间。** 这些异构数据的"动作"格式完全不同：机器人有电机控制命令（关节角度），人类视频没有（你不能让视频里的人"再打开一次抽屉"），全景扫描没有动作只有视角变化。UniSim 用 T5 语言模型将所有可能的动作信号——文本描述（"打开抽屉"）、电机控制、相机角度变化——统一嵌入到同一个连续的潜空间中。一个"打开抽屉"的语言嵌入和一个"关节 3 旋转 15°"的电机嵌入，在 UniSim 的动作空间中位于相近的位置——因为它们产生了相似的视觉后果。

**训练与推理。** UniSim 被形式化为 POMDP（部分可观测马尔可夫决策过程）中的观测预测模型——给定过去的图像序列和动作序列，预测下一帧。56 亿参数，在 512 个 TPU-v3 上训练了 20 天。在推理时，UniSim 作为一个**生成式视觉模拟器**——给定过去的观测和候选动作，生成未来的预测帧。训练好的策略可以在 UniSim 的模拟环境中零样本运行。

**直觉。** Genie 和 UniSim 代表了世界模型的**生成式转向**：在 §3 中，世界模型是为训练策略而存在的——世界模型是手段，策略是目的。Genie 和 UniSim 将世界模型本身变成了**产品**——一个生成式引擎，可以为任意策略提供无限的、可控的、多样化的训练环境，也可以直接作为内容创作工具。

### 6.2 自动驾驶世界模型

自动驾驶是世界模型最自然的应用场景之一。当你开车经过一个繁忙的十字路口，你的大脑在持续模拟：那边的行人是在等红灯还是准备穿马路？那辆打左转灯的车会不会突然并入我的车道？3 秒后我会在这个路口的什么位置？这些都是"如果我继续这样开"和"如果对方这样做"条件下的未来预测。

**GAIA-1（Wayve, 2023）。** 这是自动驾驶世界模型的一个里程碑。GAIA-1 的核心洞察是把驾驶变成一个**token 预测问题**——将 LLM 的缩放逻辑完整移植到驾驶领域。

**架构：双组件设计。**
- **世界模型（65 亿参数的自回归 Transformer）：** 接收视频 token、文本 token、动作 token，统一编码为一条 token 流，做 next-token prediction。视频 token 来自 VQ-VAE 将 194×194 的驾驶画面压缩为 16×16 的离散 token 网格。文本控制场景属性（"晴天，下午 3 点，交通繁忙"）。动作控制自车行为（"以 25mph 进入环岛"）。
- **视频扩散解码器（26 亿参数）：** 将世界模型预测的离散 token 解码回高清的、时间上连贯的视频帧。

**训练数据与缩放律。** 4700 小时的伦敦驾驶数据（2019–2023），覆盖了环岛、拥堵、窄巷、高速公路等多种场景。GAIA-1 展示的**缩放律**是这篇论文最重要的发现之一：**验证交叉熵随计算量（FLOPs）的增加而可预测地改善**——这与 LLM 的缩放律（Kaplan et al., 2020）完全平行。外推到更大的计算预算，显示显著的改善空间。

**涌现能力——GAIA-1 没有直接设计但自然出现的行为：**
- **反应性行为**：对向来车为躲避我方车辆的碰撞而主动转向——模型学会了"如果我入侵了对向车道，对方会避让"这个交互规则，而训练数据中全是专家驾驶（从不入侵对向车道）
- **3D 几何理解**：通过减速带时车身有自然的俯仰和侧倾——模型从 2D 视频中推断出了道路的 3D 高程变化
- **多未来采样**：在环岛的同一时刻，从同一起点采样"直行"和"左转"两种不同的未来——控制条件的不同 token 导致了流畅、合理的分叉场景

**OccWorld 与 OccSora（Tsinghua, 2024）。** 如果 GAIA-1 在图像空间中预测未来，OccWorld 走了一条不同的路：**在 3D 语义占用网格空间中预测。** 一个占用网格是场景的 3D 体素化表示——每个 $(x, y, z)$ 位置有一个语义标签：车辆、行人、道路、建筑物、空。

为什么要做这个转换？图像空间中的驾驶预测面临一个根本问题：视角依赖性。从相机 A 的视角看，一个行人"在画面的左上角"；从相机 B 看，同一个人"在画面的正中央"。在图像空间中预测未来，意味着你必须同时建模**物体的真实运动**和**你的相机如何移动**——两者的耦合让学习变得极其困难。3D 占用空间消除了这个耦合——无论你的相机怎么动，行人的占用格是从 $(3,4,0)$ 移动到 $(3,4.5,0)$——一个干净的 3D 位移。

**OccWorld 的具体做法：**
1. 用已有的自监督 3D 占用预测器（SelfOcc / TPVFormer / SurroundOcc）将多相机图像转化为 3D 占用网格
2. 用重建式场景分词器（类似 VQ-VAE）将占用网格压缩为离散 token
3. 用 GPT 风格的时空 Transformer 在这些离散占用 token 上自回归生成未来场景 token + 自车轨迹 token
4. 解码回占用预测（未来每个位置有什么物体）

**OccSora 走了扩散路线：** 用扩散 Transformer（而非自回归 GPT）在 4D 占用序列（3D 空间 + 1D 时间）上，以自车轨迹为条件，生成最长 16 秒的未来占用序列。扩散路线的优势是：可以在一个并行步骤中生成完整的 16 秒序列（而非逐帧自回归累积误差），并且天然支持多样性——不同的随机噪声种子产生不同的可能未来。

**直觉。** OccWorld 和 OccSora 本质上在做同一件事——**提升世界模型的抽象层次。** 像素 → VQ-VAE token → 占用标签。每一步都在丢弃更多不可预测的像素细节，保留更多可预测的语义结构。这不是巧合——这是世界模型研究的一条暗线，从 §1 的 VAE（重建所有像素）→ §3 的 VQ-VAE（压缩为 16 个 token）→ §4 的 JEPA（只预测表征）→ OccWorld（预测语义占用而非像素）。这不正是 JEPA 路线以一种不同的形式在自动驾驶领域的浮现吗？

### 6.3 具身世界模型：DayDreamer

DayDreamer（Wu et al., CoRL 2023）问了一个最实际的问题：**世界模型能不能直接在真实的物理机器人上——不是仿真器里——在线学习？**

所有 Dreamer 系列工作都在仿真器中进行。但仿真器有一个不可逾越的局限：它永远无法完美复制真实物理。摩擦系数、弹性形变、电缆的寄生效应、光线条件的微妙变化——这些要么被简化为近似公式，要么被完全忽略。在仿真器中学会的策略部署到真实机器人时，性能往往大幅下降——这就是所谓的 **sim-to-real gap**。

DayDreamer 的答案：**跳过仿真器，直接在真实硬件上用 RSSM 在线学习。** 是的，数据效率是最大的挑战——仿真器可以比实时快 10000 倍，而真实时间就是真实时间。但 DayDreamer 证明了：用了足够高效的世界模型，真实时间也可以是**可承受的**。

**实验设置。** DayDreamer 使用了 DreamerV2 的 RSSM（DreamerV3 当时尚未发布），在 4 种不同形态的机器人上测试，**所有机器人使用完全相同的超参数**：

| 机器人 | 任务 | 训练时间 | 关键挑战 |
|--------|------|---------|---------|
| **四足机器人**（A1） | 从零学习行走 | ~1 小时 | 高维连续动作空间，跌倒 = episode 结束 |
| **UR5 机械臂** | 视觉抓取-放置 | ~10 小时 | 稀疏奖励，视觉动态与机械动力学的耦合 |
| **XArm 机械臂** | 视觉抓取-放置 | ~10 小时 | 不同刚体结构，不同摩擦系数 |
| **Sphero 移动机器人** | 导航到目标 | ~2 小时 | 视觉里程计的漂移，非完整约束 |

**关键洞见。** DayDreamer 的成功不是因为 RSSM 学到了完美的物理规律（它没有），而是因为**在线学习允许 RSSM 持续适应。** 在训练的前 10 分钟，RSSM 对四足机器人的动力学几乎一无所知——它预测机器人会向前滑行，而实际是腿打滑然后摔倒。但在线收集的真实数据持续更新 RSSM，30 分钟后模型已经能粗略预测行走时的姿态变化，60 分钟后模型的想象 rollout 与真实轨迹基本一致。**世界模型不需要在部署前就完美——它可以边做边学。** 这在 §7 中将与 Dyna 范式形成呼应。

> **主线节点 6。** 2023–2025 年间，世界模型完成了从"旁观者"到"参与者"的转变。Genie 和 UniSim 证明世界模型可以成为生成式产品——为任何策略提供无限的交互式训练环境。GAIA-1 和 OccWorld 将世界模型带入了安全关键的自动驾驶应用。DayDreamer 证明了世界模型不需要仿真器——可以直接在物理硬件上在线学习。但这三条路线共享一个根本局限：**交互时长和物理准确性受限于自回归误差累积。** Genie 在 20 秒后崩溃，GAIA-1 的数分钟预测出现纹理退化，DayDreamer 的想象 rollout 误差随 16 步显著增大。下一章（§7）将展示 2025–2026 年间如何通过放弃"纯想象"和引入"平稳表征"来解决或缓解这一核心问题。
...
- **像素预测**：DreamerV3 和 IRIS 证明了 150+ 任务的跨域泛化，但纯想象训练让策略被不准确的世界模型"毒害"。世界模型的误差直接转化为策略的偏差——没有真实的纠正信号。
- **表征预测**（JEPA）：计算效率 10× 同类方法，语义和时间理解卓越，但表征不可视化——你无法直观判断世界模型是否正确。而且 JEPA 没有"想象 rollout"的能力——预测器在推理时被丢弃。
- **视频生成**：Sora 能创造逼真的视觉输出，但在分布外物理上崩溃。案例化推理替代了规则化推理。Cosmos 通过引入显式物理约束部分缓解了这一问题。

2025–2026 年间，多条线开始**合流**。Google DeepMind 的 *"Improving Transformer World Models for Data-Efficient RL"*（Dedieu et al., ICML 2025）从工程角度系统性地解决了策略-世界模型的不对齐。35 年前 Sutton 提出的 Dyna 范式重新成为最优方案。NNT 静态码书和 JEPA 的 EMA 目标编码器不约而同地指向同一个原则：平稳表征是可靠预测的前提。

### 7.1 三大改进的完整叙述

**改进 1：Dyna + 预热——Sutton 1990 年在 2025 年的回归。**

1989–1990 年间，Richard Sutton 提出了 **Dyna 架构**（Sutton, 1990）：一个 RL agent 应该同时使用**真实环境经验**和**世界模型模拟的经验**来训练策略——两者混合，而非只取其一。

在 2025 年的深度 RL 中，这个思想被基本遗忘了。标准做法（DreamerV3, IRIS）是"纯想象"——世界模型学习好之后，策略训练完全在想象的 rollouts 中进行。Dedieu et al. 发现这是现代世界模型性能最大的、但最被忽视的瓶颈。

**问题：纯想象训练的数学本质是"用近似模型替代真实环境"。** 世界模型的预测误差 $\epsilon_t = \hat{\mathbf{z}}_t - \mathbf{z}_t$ 在每个想象步中被累积。经过 $H=16$ 步想象 rollout：
$$\|\mathbf{s}_H^{\text{imagine}} - \mathbf{s}_H^{\text{real}}\| = \mathcal{O}(\epsilon \cdot H)$$
这个误差直接转化为策略的偏差——Actor 在脑海中完美执行了一个动作，但现实中因为世界模型不准确而失败了。

**Dedieu et al. 的修复——Dyna with warmup：**
- **Warmup 阶段（前 $T_{\text{BP}} = 200\text{K}$ 步）：** 策略只使用真实环境数据训练（PPO 在真实 rollouts 上）。这确保策略在接触想象数据前，已经从真实的成功和失败中学到了基础的行为模式。
- **Dyna 阶段（$T_{\text{BP}}$ 步之后）：** 策略同时使用真实数据和想象数据训练。每轮迭代中，$n_{\text{real}}$ 个真实 batch 和 $n_{\text{imagine}}$ 个想象 batch 交错训练同一个策略网络。

**Craftax-classic 上的消融实验（表中最具说服力的一列）：**

| 配置 | 奖励分数（%, 1M 步） | 解读 |
|------|---------------------|------|
| 纯想象（基线，无 Dyna） | 55.02% | 策略被不成熟的世界模型误导 |
| Dyna 无 warmup（从第 0 步就开始想象） | **33.54%** | 不成熟的世界模型产生的想象数据**比不加想象更糟** |
| **Dyna + warmup** | **67.42%** | 预热确保世界模型先成熟，然后想象数据成为有效增强 |

注意：去掉 warmup 的结果（33.54%）甚至远低于纯无模型的 PPO 基线（55.49%）。这意味着：**在错误的时间引入想象数据，不仅无益，反而有害。** 想象数据是把双刃剑——当世界模型成熟时，它是加速器；当世界模型不成熟时，它是毒药。

**直觉。** 这就像学开车。你第一天坐进驾驶座时，你的"内部世界模型"几乎为零——你对方向盘和车轮之间的关系只有模糊的直觉。如果你在第一天就闭上眼睛，完全依赖你的内部模型来"想象"驾驶——你会撞墙。但经过 20 小时的练习后，你对"打 30 度方向盘，车会转多少"有了真实的理解——此时在脑中先想一遍"如果我晚一点刹车"才是有建设性的。**Dyna + warmup 的数学本质是：让世界模型的误差 $\epsilon(t)$ 在 warmup 阶段下降到一个可接受的水平，然后再让策略依赖想象数据。**

**改进 2：最近邻分词器（NNT）——平稳码书颠覆 VQ-VAE。**

这是 Dedieu et al. 论文中最深刻但最容易被忽略的洞察。要理解它，先要理解 IRIS（§3.2）的一个隐藏问题：**VQ-VAE 的码书是非平稳的。**

在 IRIS 的训练中，VQ-VAE 的码书和 Transformer 世界模型是交替训练或联合训练的。VQ-VAE 的码书向量通过 EMA 或梯度下降持续更新——今天 token #42 代表"红车的前保险杠"，通过几轮 VQ-VAE 的更新后，token #42 可能变成了"蓝天的纹理"。这意味着 Transformer 世界模型——它的输入和输出都是这些离散 token——面对的是一个**不断移动的靶子**。序列预测任务（"给定 token #17, #42, #88，预测下一个 token 是 #..."）的语义在训练过程中不断变化。

**NNT 的解决方案——简单得令人生疑，但极有效：** 码书只增不减。
1. 将每帧分割为 patches（在 Craftax 中，9×9 网格，每个 patch 7×7 像素）
2. 对于每个 patch，计算它到现有码书中所有码字的欧氏距离
3. 如果最近距离在阈值 $\tau$ 以内：用该码字作为这个 patch 的 token
4. 如果最近距离大于 $\tau$：**将这个 patch 作为新码字加入码书**——且此后永不再更新
5. 每个 patch 被独立编码——同一帧的不同 patch 可以分属不同的码字
...
| 训练稳定性 | 需要 EMA 衰减 + 承诺损失来平衡 | 天然平稳——无需任何辅助损失 |

消融实验的贡献分解（逐步添加改进）：
```
M1（基线 IRIS-style TWM）：31.93% → M2（+Dyna）：43.36%（+11.43）
→ M3（+Patches 而非 VQ-VAE）：58.92%（+15.56）
→ M4（+NNT 静态码书）：64.96%（+6.04）
→ M5（+BTF fast）：67.42%（+2.46）
→ M5（+BTF slow）：69.66%（+2.24）
```
注意：patches + NNT 合起来的贡献（+21.60 个百分点）甚至超过了 Dyna（+11.43）。**平稳表征比想象 vs 真实的训练策略选择更重要。**

这个发现的深层含义：它证实了**表征的平稳性是世界模型训练的最大瓶颈**——不是模型容量，不是训练技巧，不是超参数。这与 §4 中 Ha & Schmidhuber 的问题——"VAE 编码了砖墙纹理却忽略了路沿"——形成了跨越 7 年的呼应。当时的问题是无法控制编码器编码什么；NNT 提供了一个不同的答案：**不控制编码的内容，但控制编码的稳定性。**

**改进 3：分块教师强制（BTF）——联合而非顺序预测。**

IRIS 的 Transformer 世界模型有一个额外的问题：在每个时间步内，下一帧的 token 是逐 token 自回归预测的——先预测 token 1，然后将 token 1 拼入上下文预测 token 2，以此类推。这有两个问题：
- **慢**：16 个 token 需要 16 次前向传播
- **自回归漂移**：token 1 的微小误差流入 token 2 的条件，token 2 的误差再流入 token 3，误差逐 token 累积

**BTF 的关键洞察：** 同一时间步的所有 token **之间不应该有因果依赖**——它们属于同一帧的不同空间区域，在物理上是同时存在的。BTF 修改了 Transformer 的注意力掩码：标准因果注意力的掩码是一个下三角矩阵（$i$ 只能注意 $j \leq i$）。BTF 的掩码是一个**分块下三角**——同一时间步内的 token 之间是双向注意（$i$ 可以注意同时间步的所有 $j$），不同时间步之间仍然是因果的（$t$ 只能注意 $\leq t$）。

这意味着在预测 $t+1$ 时刻的帧时，所有 16 个 token 被**联合并行生**成——Transformer 可以"先看完整个场景的布局，再做精细预测"，而不被强迫在逐个 token 的串行推理中猜谜。这提升了预测准确性（+2.46 个百分点），同时将想象速度提高了约 2×。

**最终结果：** 在 Craftax-classic 上仅用 1M 环境步，MBRL 达到 69.66% 奖励，**首次超越了人类专家（65.0% ± 10.5%）**。在 Craftax-full 上达到 7.20%（之前 SOTA 为 6.59%）。更有说服力的是：无模型 PPO 基线（55.49%，15 分钟训练）已经超过了他们复现的 DreamerV3（53.2%，35 小时训练）——提示之前许多报告的"MBRL 增益"实则来源于**弱的无模型基线**，而非世界模型本身的优势。

### 7.2 Cosmos 3：统一的物理 AI 平台

NVIDIA 在 2026 年推出的 Cosmos 3 代表了合流的另一个维度：**统一生成-推理-动作的单一架构。** Cosmos Predict（未来视频生成）、Cosmos Transfer（Sim2Real 变换）、Cosmos Reason（物理推理 VLM）不再作为三个独立模型存在，而是成为一个联合训练的架构的不同能力头。

这是一个重要的建筑设计趋势：**世界模型正在从"预测工具"演变为"通用物理 AI 平台"**——它们不仅能预测未来（Predict），还能推理为什么（Reason），能在不同模态间转换（Transfer），并能直接输出动作（Action）。这与 LeCun 在 §2 中设想的六模块认知架构——世界模型 + Cost + Actor + Configurator + Perception + Memory——在结构上高度吻合，只是实现方式不同：LeCun 设想的是 JEPA 路线的不生成表征，Cosmos 实现的是物理知情的生成。

### 7.3 合流的方向：三个原则的共识

综合 §7.1 和 §7.2 的进展，世界模型研究在 2025–2026 年间形成了一个新的共识——三个原则贯穿了看似独立的不同路线：

**原则 1：表征必须是平稳的。** NNT 的静态码书（§7.1 改进 2）和 JEPA 的 EMA 目标编码器（§4.1 决策 2）在本质上解决了同一个问题：给预测器一个不移动的靶子。NNT 通过"创建后永不更新"实现，JEPA 通过"慢速追踪 $\text{Enc}_c$"实现。VQ-VAE 的非平稳码书被证明是 IRIS 风格方法的核心瓶颈——平稳目标比模型容量或训练技巧更重要。

**原则 2：纯想象不如真假混合。** Dyna 的回归（§7.1 改进 1）是这次合流的最深刻洞察。DreamerV3 和 IRIS 的纯想象训练被证明只在一个狭窄条件下有效：世界模型已经足够准确。当世界模型还不成熟时，想象数据不仅无用而且有害——比不加想象更糟。真实环境交互是不可替代的——世界模型的数学本质是 $\mathcal{O}(\epsilon \cdot H)$ 的误差累积，而唯一能打破这个累积的，是定期回到真实世界重新校准。Dyna 的本质不是"世界模型足够好时可以替代真实"，而是"世界模型是真实的加速器，不是替代品"。

**原则 3：物理不能仅从数据中学到。** Sora 在分布外物理上的系统失败（§5.2）和 Cosmos 通过 PhysX 注入物理约束的相对成功（§5.3），以及 Kang et al. 的"颜色 > 速度"优先级排序，共同指向一个结论：纯神经网络——无论多大——从视频数据中学习物理的"归纳偏置"不足以支持分布外泛化。未来的世界模型可能不会是纯神经网络——它们会融合可微分物理求解器（如 PhysX/Genesis）、符号推理模块（如 Cosmos-Reason）、和神经网络生成器。

> **主线节点 7。** 2025–2026 年间，世界模型研究的三条主要路线——像素预测、表征预测、视频生成——开始合流。Dyna 范式（真假混合训练）在 35 年后回归，证明纯想象训练有一个硬性的误差累积天花板。NNT 静态码书和 JEPA 的 EMA 编码器不约而同地指向"平稳表征是可靠预测的前提"——表征的不平稳性被证明是比模型容量更根本的瓶颈。物理知情生成（Cosmos）为"物理不能仅从数据中学到"的康德式论证提供了实证解决方案。合流的最终方向不是"哪个路线赢"，而是三个路线互补优势的整合：**JEPA 的高效表征（不浪费容量在纹理上）+ 重建式模型的生成能力（可视化验证）+ 显式物理约束（分布外泛化）。**
## 8. 开放问题与未来方向
