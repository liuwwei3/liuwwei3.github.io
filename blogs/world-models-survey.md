...
本文的 §1、§2 和 §7 都在处理这个核心权衡：**如何学习一个足够好的世界模型，使得在想象中训练的策略能在真实世界中泛化？**
...
> **主线节点 2。** 像素/Token 世界模型在 2023 年被推到了极致。DreamerV3 证明它们可以跨 150+ 个任务泛化。IRIS 证明 Transformers 是有效且样本高效的世界模型。但根本的痛点仍未解决：**世界模型被迫编码与任务无关的视觉细节，策略被不准确的世界模型误导。** 这两个痛点分别被 §4（JEPA 的表征空间预测）和 §7（Dyna 的混合训练）分别解决。
...
在逐条展开之前，先理解 I-JEPA 的遮盖机制本身——因为三条设计决策都与它紧密相关。I-JEPA 对每张图像的处理方式是：

1. 随机选择 **4 个目标块**（$\mathbf{y}$，即 §3.2 中 JEPA 公式里的"目标"），每个覆盖 15–20% 的图像面积，宽高比在 0.75–1.5 之间随机变化（目标块之间不允许重叠）
2. **上下文块**（$\mathbf{x}$，即公式里的"上下文"）= 整张图像**减去**这 4 个目标块——上下文编码器 $\text{Enc}_c$ 看到的是一个"镂空"的图像，4 个矩形区域被挖掉
3. **目标编码器** $\text{Enc}_t$ 看到的是**完整的、未被遮盖的原始图像**
4. 损失 $\mathcal{L} = \|\tilde{\mathbf{s}}_y - \mathbf{s}_y\|^2$ 只计算被遮盖的那 4 个目标块区域对应的 patch 表征——其余区域（上下文编码器能看到的 85–100% 的图像面积）不参与损失

有了这个遮盖方案作为前提，三条设计决策的意义才能被充分理解。
...
**直觉。** Genie 和 UniSim 代表了世界模型的**生成式转向**：在 §2 中，世界模型是为训练策略而存在的——世界模型是手段，策略是目的。Genie 和 UniSim 将世界模型本身变成了**产品**——一个生成式引擎，可以为任意策略提供无限的、可控的、多样化的训练环境，也可以直接作为内容创作工具。
...
**直觉。** OccWorld 和 OccSora 本质上在做同一件事——**提升世界模型的抽象层次。** 像素 → VQ-VAE token → 占用标签。每一步都在丢弃更多不可预测的像素细节，保留更多可预测的语义结构。这不是巧合——这是世界模型研究的一条暗线，从 §1 的 VAE（重建所有像素）→ §2 的 VQ-VAE（压缩为 16 个 token）→ §3 的 JEPA（只预测表征）→ OccWorld（预测语义占用而非像素）。这不正是 JEPA 路线以一种不同的形式在自动驾驶领域的浮现吗？
...
走到 2025 年，世界模型研究的三个主要路线——像素预测（§2）、表征预测（§3, §4）和视频生成（§5）——各自展现了优势，也暴露了互补的弱点：
...
这是 Dedieu et al. 论文中最深刻但最容易被忽略的洞察。要理解它，先要理解 IRIS（§2.2）的一个隐藏问题：**VQ-VAE 的码书是非平稳的。**
...
这是一个重要的建筑设计趋势：**世界模型正在从"预测工具"演变为"通用物理 AI 平台"**——它们不仅能预测未来（Predict），还能推理为什么（Reason），能在不同模态间转换（Transfer），并能直接输出动作（Action）。这与 LeCun 在 §3 中设想的六模块认知架构——世界模型 + Cost + Actor + Configurator + Perception + Memory——在结构上高度吻合，只是实现方式不同：LeCun 设想的是 JEPA 路线的不生成表征，Cosmos 实现的是物理知情的生成。
