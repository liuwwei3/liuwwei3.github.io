...
**但暴露了"双重探索问题"：** Agent 必须先在真实环境中发现新游戏机制让世界模型学习，然后策略在想象中重新发现它们。在有罕见关卡转换的游戏（如 Frostbite）上，世界模型从未见过后续关卡 → 策略在想象中无法学习这些关卡的策略 → 性能崩溃。这引出 §6.5 的核心命题：**纯想象训练是不够的。**
...
> **BTF** = **B**lock **T**eacher **F**orcing。**Teacher Forcing**（教师强制）是序列建模中的标准训练技术——在训练时，将真实的前一步输出（而非模型自己的预测）作为下一步的输入，避免误差级联。Block 指将同一时间步的所有 token 视为一个整体块，在块内使用双向注意力联合预测，而非逐 token 串行生成。简单说，BTF 就是"逐块"而非"逐 token"的教师强制——每个时间块作为一个整体被一次性预测出来。
...
合流之后，世界模型研究的下一阶段面临四个核心问题。

**层次化规划。** LeCun 设想的 H-JEPA——在多个时间尺度上堆叠世界模型，高层输出作为低层的子目标——至今没有任何完整的实现。DreamerV3 在单一时间尺度上运作，Genie 的自回归生成本质上也是单一尺度的。如何学习层次化的时间抽象，如何在不同尺度上联合优化——这些仍然是完全开放的问题。

**评估基准。** 当前的世界模型缺乏单一权威的评估基准。WorldSimBench、VBench、PhysBench、EWMBench 各自衡量不同维度。一个理想的基准应该同时测量物理准确性（是否违反物理定律）、长期一致性（100+ 步后是否仍然合理）、交互响应性（对动作输入的响应是否正确）、以及反事实推理（是否能在分布外场景中做出合理预测）。

**Configurator。** LeCun 将其称为"最神秘的"模块——Configurator 如何根据当前任务动态地配置世界模型的所有其他组件？在某种意义上，这就是 AGI 本身：一个可以通用地适应任何任务的系统。DreamerV3 用固定超参数实现了跨任务泛化的第一步，但这距离有意识地动态调整内部配置还有本质的距离。

**安全与伦理。** 随着世界模型变得足够强大——超逼真的 deepfake 视频、自主武器系统的一部分、自动驾驶世界模型直接输出决策——安全与伦理框架的建设需要与技术进步同步。NVIDIA Cosmos 这类公开可用的物理 AI 平台使这些问题的紧迫性进一步提升。理解世界意味着理解因果；而理解因果意味着承担因果的责任。
